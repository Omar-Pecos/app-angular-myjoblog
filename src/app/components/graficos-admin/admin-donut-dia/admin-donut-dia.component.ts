...
@Component({
  selector: 'admin-donut-dia',
  templateUrl: './admin-donut-dia.component.html',
   providers:[UserService,GraphService]
})

export class AdminDonutDiaComponent implements OnChanges{
  public title = 'Horas trabajadores / 8 horas (Hoy)';
  public identity;
  public token;
  public loading = true;
 ...
 public donutChartLabels = ['8 horas'];
 ...
 constructor(
                 ...
                 this._graphService.data_donut_dia(this.token,this.oldusers[0]).subscribe(
                        response => {
                                ...
                                //     LA SEGUNDA LLAMADA A LOS DATOS CON EL SEGUNDA DE LOS OLDUSERS ---> SERÁ EL 2
                                       this._graphService.data_donut_dia(this.token,this.oldusers[1]).subscribe(
                                                  response => {
  ...
  ngOnChanges(changes: SimpleChanges){
     ...
     if (this._privId != '-1' && this.oldusers.includes(parseInt(this._privId)) == false && this._privColor != '0'){
            ...
            this.loading = true;
            this.AddAdminDonutDiadata(this._privId,this._privColor);
      }
    
}


  AddAdminDonutDiadata(id,color){
    this.loading = true;
    // var color = this.GenColor();
     var colorrgb = 'rgb('+color.r+','+color.g+','+color.b;

        this._graphService.data_donut_dia(this.token,id).subscribe(
                response => {
